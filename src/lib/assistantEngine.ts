...
function buildIntents(): Intent[] {
  return [
    ...
    // --- Data Engineering ---
    {
      ...
      handler: () => ({
        text: `Pradeep has significant data engineering experience:\n\n- At Cognizant, engineering Python/SQL data services that migrate legacy SQL Server workloads to GCP BigQuery and Dataflow, processing 10M+ records monthly and cutting downstream data latency by about 30%\n- At BMSC, built PySpark/Spark SQL pipelines on AWS and Databricks to migrate ~500 GB of SQL Server data and process 2M+ records monthly\n- At Shree Kay Vee, built pipelines for telemetry from 100+ industrial assets, reducing data-processing latency by 40%\n- At CEAC, built Python/SQL workflows serving 5,000+ stakeholders, improving data reliability by 20%\n\nTools & technologies:\nAWS (S3, Glue, Lambda), GCP (BigQuery, Dataflow), Apache Spark/PySpark, Databricks, Docker, Kubernetes, SQL Server\n\nHe holds AWS Cloud Architecting certification and has hands-on experience across the AWS ecosystem.`,
